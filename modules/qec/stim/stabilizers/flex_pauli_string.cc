...
FlexPauliString::FlexPauliString(size_t num_qubits) :
		value(num_qubits), imag(false) {
}

FlexPauliString::FlexPauliString(const PauliStringRef<MAX_BITWORD_WIDTH> val, bool imag) :
		value(val), imag(imag) {
}

FlexPauliString::FlexPauliString(PauliString<MAX_BITWORD_WIDTH> &&val, bool imag) :
		value(std::move(val)), imag(imag) {
}
